...
def test_basic_evaluation(tmp_path, resources):
    ...
    assert (tmp_path / "website" / "Euroc" / "detailed_performance.html").exists()
    assert (tmp_path / "website" / "Euroc" / "frontend.html").exists()
    assert (tmp_path / "website" / "Euroc" / "datasets.html").exists()
