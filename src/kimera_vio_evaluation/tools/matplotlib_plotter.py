"""Plot information via matplotlib."""
import os
import numpy as np
...
def draw_rpe_boxplots(output_dir, stats, n_segments):
    ...
    if isinstance(stats, dict):
        ...
        pos = np.arange(
            0, n_segments * (n_experiment + spacing), (n_experiment + spacing)
        )
        ...
        ax_yaw = fig.add_subplot(
            212, ylabel="RPE rotation [deg]", xlabel="Distance travelled [m]"
        )
        dummy_plots_pos = []
        dummy_plots_yaw = []
        ...
        for pipeline_key, errors in sorted(stats.items()):
            ...
            if isinstance(errors, dict):
                ...
                idx_segment = 0
                for segment_length, stats in sorted(
                    errors["relative_errors"].items(), key=lambda item: int(item[0])
                ):
                    ...
                    draw_boxplot(
                        ax_pos,
                        stats["rpe_trans"],
                        [idx_experiment + pos[idx_segment]],
                        idx_experiment,
                    )
                    draw_boxplot(
                        ax_yaw,
                ...
                raise Exception(
                    "\033[91mValue in stats should be a dict: " + errors + "\033[99m"
                )
        ...
        def _ax_formatting(ax, dummy_plots, final_max_e):
            ...
            if len(yticks) < 4:  # 4 is the minimum of yticks that we want.
                ax.set_yticks(
                    np.arange(0, final_max_e, find_step_of_base(final_max_e / 5, 1))
                )
            else:
        ...
        fig.savefig(
            os.path.join(output_dir, "traj_relative_errors_boxplots.pdf"),
            bbox_inches="tight",
            format="pdf",
            dpi=1200,
        )
...
def draw_ape_boxplots(stats, output_dir):
    ...
    if isinstance(stats, dict):
        ...
        pos = np.arange(
            0,
            n_param_values * (n_pipeline_types + spacing),
        ...
        ax_pos = fig.add_subplot(
            111, ylabel="APE translation error [m]", xlabel="Dataset"
        )
        legend_labels = []
        legend_handles = []
        # Draw legend.
        ...
        for dataset_name, pipeline_types in sorted(stats.items()):
            xtick_labels.append(dataset_name.replace("_", r"\_"))
            if isinstance(pipeline_types, dict):
                ...
                for pipeline_type, pipeline_stats in sorted(pipeline_types.items()):
                    if isinstance(pipeline_stats, dict):
                        # Find max value overall, to set max in y-axis
                        assert isinstance(
                            pipeline_stats["absolute_errors"], result.Result
                        )
                        ...
                        draw_boxplot(
                            ax_pos,
                            pipeline_stats["absolute_errors"].stats,
                            [idx_pipeline_type + pos[idx_param_value]],
                            idx_pipeline_type,
                        )
                    else:
                        # If pipeline_stats is not a dict, then it means
                        # the pipeline failed...
                        # Just plot a cross...
                        pipelines_failed[idx_pipeline_type] = [
                            pipeline_type,
                            idx_param_value,
                        ]
        ...
        fig.savefig(
            os.path.join(output_dir, "datasets_ape_boxplots.pdf"),
            bbox_inches="tight",
            format="pdf",
            dpi=1200,
        )
...
def draw_regression_simple_boxplot_APE(param_name, stats, output_dir, max_y=-1):
    """
    Draw regression box plots.

    Draw boxpots where x-axis are the values of the parameter `param_name`, and the
    y-axis has boxplots with APE performance for the different pipelines represented in
    stats, where stats is defined as below:

    Args:
        param_name: regressed parameter name
        stats: is organized as follows:
            - param_value_dir (path to directory containing results for the parameter)
                - pipeline (pipeline type e.g. S, SP or SPR)
                    - results (which is actually -max, -min etc !OR! False.
        output_dir:
        max_y:
    """
    colors = ["blue", "black", "green", "red", "mangenta", "cyan", "orange"]
    ...
    # Precompute position of boxplots in plot.
    pos = np.arange(
        0, n_param_values * (n_pipeline_types + spacing), (n_pipeline_types + spacing)
    )
    ...
    ax_pos = fig.add_subplot(
        111,
        ylabel="APE translation error [m]",
        xlabel="Values of parameter: {}".format(param_name),
    )
    legend_labels = []
    legend_handles = []
    # Draw legend.
    color_id = 0
    for pipeline_type, pipeline_stats in sorted(
        list(stats.values())[0].items()
    ):  # inefficient in py2 but compatible py2 & 3
        # The dummy plots are used to create the legends.
        dummy_plot_pos = ax_pos.plot([1, 1], "-", color=colors[color_id])
        legend_labels.append(pipeline_type)
        legend_handles.append(dummy_plot_pos[0])
    ...
    for param_value_boxplots, pipeline_types in sorted(stats.items()):
        assert isinstance(pipeline_types, dict)
    ...
    fig.savefig(
        os.path.join(output_dir, param_name + "_absolute_errors_boxplots.pdf"),
        bbox_inches="tight",
        format="pdf",
        dpi=1200,
    )
