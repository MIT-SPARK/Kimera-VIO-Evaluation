...
def parser():
    ...
    input_options.add_argument(
        "vio_results_path",
        help="Path to the **YAML** file containing the VIO results.",
        default="./results/V1_01_easy/S/results.yaml",
    )
    input_options.add_argument(
        "vio_results_summary_path",
        help="Path to the **CSV** file containing the VIO summary results.",
...
def write_vio_results_summary(results, vio_results_summary_path):
    ...
    with open(vio_results_summary_path, "w") as vio_results_summary_file:
        ...
        writer = csv.DictWriter(
            vio_results_summary_file, fieldnames=performance_metrics
        )
        writer.writeheader()
