...
def draw_regression_simple_boxplot_APE(param_name, stats, output_dir, max_y = -1):
    """ Draw boxpots where x-axis are the values of the parameter `param_name`, and the y-axis has boxplots with APE
    performance for the different pipelines represented in stats, where stats is defined as below:
    ...
    ax_pos = fig.add_subplot(111, ylabel='APE translation error [m]', xlabel="Values of parameter: {}".format(param_name))
    legend_labels = []
    legend_handles = []
    # Draw legend.
    color_id = 0
